...
class EvaluationViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        evalutor_type = 'workplace' if user.role == 'workplace_supervisor' else 'academic'

        serializer.save(
            evalutor=user,
            evalutor_type=evalutor_type
        )

    @action(detail=False, methods=['get'], url_path=r'summary/(?P<placement_id>[^/.]+)')
    def summary(self, request, placement_id=None):
        try:
            placement = InternshipPlacement.objects.select_related('student').get(pk=placement_id)
        ...
        max_possible = (EvaluationCriteria.objects.aggregate(
